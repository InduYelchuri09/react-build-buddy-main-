...
export const getNetwork = createServerFn({ method: "GET" })
  .validator((input: { region?: string | null; limit?: number }) => ({
...
export const getImpact = createServerFn({ method: "POST" })
  .validator((input: { supplierId: string; maxHops?: number }) => {
